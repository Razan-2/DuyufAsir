...
const agents = {
    ...
    housing: { icon: "fa-house", listTitle: "السكن والفنادق ونُزل الإيواء التراثية في عسير", features: [
        ["fa-magnifying-glass", "البحث عن فنادق ونُزل تراثية"],
        ...
        { name: "نُزل الحصون التراثية", location: "قرية رجال ألمع التراثية – رجال ألمع", people: "أفراد وعائلات", rooms: "إقامة في حصون حجرية بطابع عسيري", nightly: "السعر والتوفر لدى مقدم الإقامة", nightlyRate: 0, monthly: "تجربة إقامة تراثية", monthlyRate: 0, image: "assets/properties/rijal-almaa-heritage.jpg", mapUrl: "https://www.google.com/maps/search/?api=1&query=نزل+الحصون+التراثية+رجال+ألمع", bookingUrl: "https://ar.airbnb.com/rijal-almaa-saudi-arabia/stays", bookable: true, heritageStay: true },
        { name: "نُزل قرية عنقرة التراثية", location: "قرية عنقرة – جنوب شرق أبها", people: "أفراد وعائلات", rooms: "غرف تراثية وتجربة ريفية جنوبية", nightly: "السعر والتوفر لدى مقدم الإقامة", nightlyRate: 0, monthly: "تجربة ريفية وتراثية", monthlyRate: 0, image: "assets/properties/rijal-almaa-heritage.jpg", mapUrl: "https://www.google.com/maps/search/?api=1&query=قرية+عنقرة+عسير", bookingUrl: "https://www.google.com/maps/search/?api=1&query=قرية+عنقرة+عسير", bookable: true, heritageStay: true },
...
function renderRecommendations(agent, educationGroup = "universities") {
    ...
    if (agent === agents.transport) {
        ...
        controls.innerHTML = `
            <div class="housing-search-row"><input id="housingSearch" type="search" value="${escapeHtml(housingState.query)}" placeholder="ابحث عن فندق أو نُزل تراثي أو شقة" aria-label="البحث عن سكن"><button type="submit"><i class="fa-solid fa-magnifying-glass"></i><span>بحث</span></button></div>
        ...
        visibleItems.sort((first, second) => Number(Boolean(second.heritageStay)) - Number(Boolean(first.heritageStay)));
    ...
    recommendationsList.classList.toggle("guide-directory-list", agent === agents.guides);
    ...
    visibleItems.forEach((entry) => {
        ...
        button.className = transportCard ? "recommendation-item transport-vehicle-card" : rich ? "recommendation-item property-item" : visual ? `recommendation-item visual-option-item ${entry.imageType || "photo"}${entry.featuredLocal ? " productive-family-card" : ""}` : `recommendation-item${agent === agents.guides ? " guide-directory-item" : ""}`;
